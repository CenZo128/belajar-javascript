        ...
        for (var j = i + 1; j < names.length; j++) {
          if (names[j]) {
            if (names[i][0] !== names[j][0]) {
              result.push("".concat(names[i], " dan ").concat(names[j]));
              names[i] = false;
              names[j] = false;
              break;
            }
...
console.log(pairProject(["Afifah", "Bayu", "Juminten", "Junaedi", "Iqbal", "Marimar"])); // ["Afifah dan Bayu", "Juminten dan Iqbal", "Junaedi dan Marimar"]

console.log(pairProject(["Afifah", "Bayu", "Juminten"])); // jumlah murid harus genap