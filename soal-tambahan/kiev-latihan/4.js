...
function getProductsAmount(productsArr) {
  ...
  function shoppingTeros(customer) {
    ...
    if (customer.member === true) {
      return `Hai pelanggan setia ${customer.name}! Total Harga yang harus kamu bayar adalah Rp ${diskon}`
    } else {
      return `Hai ${customer.name}! Total Harga yang harus kamu bayar adalah Rp ${totalHarga}`
    }
  }
